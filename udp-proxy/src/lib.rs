...
const JITO_ENDPOINT_STATUS: &str = "https://ny.mainnet.block-engine.jito.wtf/api/v1/getBundleStatuses";
...
impl BundleSender {
    ...
    pub fn bundle_status(&self, bundle_id: String) {
        let payload = json!({
            "jsonrpc": "2.0",
        ...
        self.tokio_runtime.block_on(async move {
            let res = client
            .post(JITO_ENDPOINT_STATUS)
            .header("Content-Type", "application/json")
            .json(&payload) // Serialize the payload to JSON
            .send()
            .await;
            if res.is_ok() {
                println!("bundle status {:#?}", res.unwrap().text().await);
            }
            else {
                println!("bundle status Error {:#?}", res.unwrap().text().await);
            }
            
        });
    }
    pub fn send_bundle(&self, versioned_txs: &[VersionedTransaction], self_only: bool) {
        ...
        if counter == 0 || counter == 1 || self_only { // self sending
            self.send_bundle_self(&payload, JITO_ENDPOINT_NY);
        }
        else {
